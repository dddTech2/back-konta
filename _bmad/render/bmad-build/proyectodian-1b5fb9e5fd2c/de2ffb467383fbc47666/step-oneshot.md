...
If `{story_key}` is not empty and `C:\Users\dazad\OneDrive\Documentos\ProyectoDian/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `C:\Users\dazad\OneDrive\Documentos\ProyectoDian\_bmad\render\bmad-build\proyectodian-1b5fb9e5fd2c\de2ffb467383fbc47666\sync-sprint-status.md` with `{target_status}` = `in-progress`.
...
Write what triggered the stop in `## Implementation Notes`. Then update `{spec_file}`: add back `## Code Map` (filled in from what you learned while implementing) and `## Open Questions` (one question per gap), set `route: 'full'` and `status: 'draft'`. Go back to `C:\Users\dazad\OneDrive\Documentos\ProyectoDian\_bmad\render\bmad-build\proyectodian-1b5fb9e5fd2c\de2ffb467383fbc47666\step-02-plan.md` step 6.
...
If a layer needs subagents and you cannot launch them, write the full prompt for each layer under `C:\Users\dazad\OneDrive\Documentos\ProyectoDian/_bmad-output/implementation-artifacts` (with placeholders filled in, not just file paths). Stop and ask the user to run each prompt in a separate session and paste back the findings.
...
If `{story_key}` is not empty and `C:\Users\dazad\OneDrive\Documentos\ProyectoDian/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `C:\Users\dazad\OneDrive\Documentos\ProyectoDian\_bmad\render\bmad-build\proyectodian-1b5fb9e5fd2c\de2ffb467383fbc47666\sync-sprint-status.md` with `{target_status}` = `review`.
